...
USER_AGENT_PARTS = {
    'os': {
        ...
        'mac': {
            'name': ['Macintosh'],
            'ext': ['Intel Mac OS X %d_%d_%d' % (random.randint(53, 55), random.randint(58, 60), random.randint(0, 5)) for i in range(63, 72)]
        },
    },
    'platform': {
        'webkit': {
            ...
            'extensions': ['Chrome/%d.0.%d.%d Safari/%d.%d' % (random.randint(6, 32), random.randint(100, 2000), random.randint(50, 100), random.randint(535, 537), random.randint(1, 36)) for i in range(1, 30) ] + [ 'Version/%d.%d.%d Safari/%d.%d' % (random.randint(4, 6), random.randint(0, 1), random.randint(0, 9), random.randint(535, 537), random.randint(1, 36)) for i in range(1, 10)]
        },
        'iexplorer': {
            'browser_info': {
        ...
        'gecko': {
            'name': ['Gecko/%d%02d%02d Firefox/%d.0' % (random.randint(2001, 2010), random.randint(50, 100)
            'extensions': []
        }
    }
}
...
class Striker(Process):
    ...
    def __init__(self, url, nr_sockets, counter):
        ...
        if not self.port:
            self.port = 80,53,22 if not self.ssl else 443
    ...
    def generateRandomHeaders(self):
        ...
        http_headers = {
            ...
            'Keep-Alive': random.randint(50,1000),
            'Host': self.host,
        }
